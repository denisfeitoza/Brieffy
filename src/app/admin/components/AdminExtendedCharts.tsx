...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  pending: { label: 'Pending', color: 'text-[var(--text2)] bg-zinc-400/10 border-zinc-400/20', icon: AlertCircle },
};
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        ...
        {/* Blocked Users */}
        <Card className={`bg-[var(--bg2)] ${blockedCount > 0 ? 'border-red-500/20' : 'border-[var(--bd)]'}`}>
          <CardHeader className="pb-2 pt-4 px-4 md:px-6">
            <CardTitle className="text-xs md:text-sm text-[var(--text2)] font-normal flex items-center gap-1.5">
              <AlertTriangle className={`w-3.5 h-3.5 ${blockedCount > 0 ? 'text-red-400' : 'text-[var(--text3)]'}`} />
              Blocked Users
            </CardTitle>
          </CardHeader>
          <CardContent className="px-4 md:px-6 pb-4">
            <p className={`text-2xl md:text-3xl font-bold ${blockedCount > 0 ? 'text-red-400' : 'text-[var(--text2)]'}`}>
              {blockedCount}
            </p>
            ...
            <div className="h-[220px] w-full mt-2">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={newUsersWeekly} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
                  ...
                  />
                  <Area type="monotone" dataKey="count" stroke="#ff6029" strokeWidth={2.5} fill="url(#usersGrad)" dot={{ r: 4, fill: '#ffffff', strokeWidth: 2 }} activeDot={{ r: 6 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] font-medium border ${
                          ...
                          user.plan === 'pro' ? 'text-[var(--orange)] bg-[var(--bg2)] border-[var(--bd-strong)]' :
                          'text-[var(--text2)] bg-zinc-400/10 border-zinc-400/20'
                        }`}>
                          {user.plan || 'free'}
                        </span>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link href={`/admin/users/${user.id}`}>
                        <Button variant="ghost" size="sm" className="text-[var(--orange)] hover:text-[var(--orange)] rounded-lg text-xs px-2 sm:px-3">
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={userPage === 0}
                className="border-[var(--bd)] text-[var(--text2)] hover:bg-white/5 rounded-xl h-8 px-3"
              >
