    ...
    <>
      <motion.nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled
            ? "bg-white/80 backdrop-blur-2xl border-b border-gray-200 shadow-xl shadow-black/5"
            : "bg-transparent"
        }`}
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <Link href="/" className="flex items-center gap-2.5 group">
              <motion.div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, #ff6029, #ffcfbc)",
                }}
              ...
              >
                ...
                <span
                  style={{
                    background:
                      "linear-gradient(90deg, #ff6029, #ffcfbc)",
            ...
            <div className="hidden md:flex items-center gap-8">
              {NAV_LINKS.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="text-sm text-neutral-600 hover:text-black transition-colors duration-300 relative group"
                >
                  {link.label}
                  <span className="absolute -bottom-1 left-0 w-0 h-[2px] bg-gradient-to-r from-[#ff6029)] to-[#ffcfbc] group-hover:w-full transition-all duration-300 rounded-full" />
                </a>
              ))}
            </div>
            ...
            <div className="hidden md:flex items-center gap-3">
                ...
                {(['pt', 'en', 'es'] as Language[]).map((lang) => (
                  <button
                    ...
                    className={`flex items-center justify-center w-6 h-6 rounded-full transition-all ${
                      language === lang ? 'bg-black/10 scale-110' : 'hover:bg-black/5 opacity-60 hover:opacity-100'
                    }`}
              ...
              <Link
                ...
                className="text-sm text-neutral-600 hover:text-black transition-colors px-4 py-2"
              >
                {t("nav.login")}
              </Link>
              <Link
              ...
              >
                ...
                <span
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, #ffcfbc, #ff6029)",
                  }}
                />
      ...
      {/* Mobile Menu */}
      <motion.div
        className="fixed inset-0 z-40 bg-white/98 backdrop-blur-3xl flex flex-col items-center justify-center gap-6 md:hidden"
        initial={false}
        animate={
          mobileOpen
      ...
      >
        ...
        <div className="flex flex-col gap-3 mt-4 w-56">
          <Link
            href="/dashboard/login"
            className="text-center text-sm text-neutral-600 border border-black/10 rounded-full py-3 hover:bg-black/5 transition-colors"
            onClick={() => setMobileOpen(false)}
          >
            {t("nav.login")}
          </Link>
